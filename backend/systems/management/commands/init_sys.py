...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            user = User.objects.create(username='admin', password=make_password("qwert@12345"), group=group,
                                       is_admin=True)
            user.roles.add(role)
        except:
        ...
        if len(menus) == 0:
            ...
            init_menu(menumodel)
            menumodel = Menu.objects.create(name='用户组管理', code='group', curl='/group', icon='group', sequence=20,
                                            type=2,
                                            parent_id=sysmenu.id)
            init_menu(menumodel)
