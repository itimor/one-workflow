...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        menumodel = Menu.objects.create(name='新建工单', code='c_ticket', curl='/c_ticket', icon='c_ticket', sequence=10, type=2,
                                        parent_id=ticketmenu.id)
        init_menu(menumodel)
        menumodel = Menu.objects.create(name='编辑工单', code='u_ticket', curl='/u_ticket/:id', icon='u_ticket', sequence=20, type=2,
                                        hidden=True, parent_id=ticketmenu.id)
        init_menu(menumodel)
        menumodel = Menu.objects.create(name='我的工单', code='my_ticket', curl='/my_ticket', icon='my_ticket', sequence=30, type=2,
                                        parent_id=ticketmenu.id)
        init_menu(menumodel)
        menumodel = Menu.objects.create(name='所有工单', code='all_ticket', curl='/all_ticket', icon='all_ticket', sequence=40, type=2,
                                        parent_id=ticketmenu.id)
        init_menu(menumodel)
